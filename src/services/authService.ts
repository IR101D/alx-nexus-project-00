import { RegisterRequest, RegisterResponse, LoginRequest, LoginResponse } from "@/interfaces";
...
export const login = async (payload: LoginRequest): Promise<LoginResponse> => {
  ...
  try {
    ...
    if (!res.ok) {
      try {
        const data = await res.json();
        return { message: data?.message || "Sign in failed. Please try again." };
      } catch {
        const text = await res.text();
        return { message: text || "Sign in failed. Please try again." };
      }
    }
